	...
	public static Object getJson(Object javaBean) {
		...
		if (null != javaBean) {
			try {
				...
				/* 对实体进行脱敏操作 */
				// DesensitizedUtils.replace(ObjectUtils.getAllFields(javaBean), javaBean,
				// referenceCounter);
						...
						if (type.isArray()) {// 对数组类型的字段进行递归过滤
							...
							for (int i = 0; i < len; i++) {
								...
								if (isNotGeneralType(arrayObject.getClass(), arrayObject, referenceCounter)) {
									// replace(ObjectUtils.getAllFields(arrayObject), arrayObject,
									// referenceCounter);
								}
							}
							...
							while (it.hasNext()) {// TODO: 待优化
								...
								if (isNotGeneralType(collectionObj.getClass(), collectionObj, referenceCounter)) {
									// replace(ObjectUtils.getAllFields(collectionObj), collectionObj,
									// referenceCounter);
								}
							}
